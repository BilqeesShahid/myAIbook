#!/usr/bin/env python3
"""
Script to process the docs folder and create local Hugging Face embeddings for the RAG system.
This script reads all markdown files from the docs folder, chunks them,
generates local sentence transformer embeddings, and stores them in Qdrant.
"""
...
def process_docs_folder(docs_path: str = "docs"):
    """
    Process all markdown files in the docs folder and create local Hugging Face embeddings.
    """
    ...
    # Store embeddings in Qdrant using local Hugging Face embeddings
    try:
        ...
        logger.info("Local Hugging Face embedding process completed successfully!")
        return True
...
if __name__ == "__main__":
    ...
    if not success:
        logger.error("Document local Hugging Face embedding process failed.")
        sys.exit(1)
    else:
        logger.info("Document local Hugging Face embedding process completed successfully!")