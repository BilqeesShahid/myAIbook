#!/usr/bin/env python3
"""
Script to process both English and Urdu docs folders and create embeddings for the RAG system.
This script reads all markdown files from both the docs and i18n folders, chunks them,
...
def process_docs_folder(docs_path: str, language: str = "en"):
    """
    Process all markdown files in a folder and create embeddings.
    """
...
if __name__ == "__main__":
    ...
    # Process English documents first
    logger.info("Processing English documents...")
    english_success = process_docs_folder(docs_path, "en")

    # Process Urdu documents
    logger.info("Processing Urdu documents...")
    urdu_success = process_docs_folder(i18n_path, "ur")

    if not english_success:
        logger.error("English document embedding process failed.")

    if not urdu_success:
        logger.error("Urdu document embedding process failed.")

    if english_success and urdu_success:
        logger.info("Both English and Urdu document embedding processes completed successfully!")
        sys.exit(0)
    else:
        logger.error("One or more embedding processes failed.")
        sys.exit(1)