#!/usr/bin/env python3
"""
Script to process the docs folder and create TF-IDF embeddings for the RAG system.
This script reads all markdown files from the docs folder, chunks them,
generates local TF-IDF embeddings, and stores them in Qdrant.
"""
...
def process_docs_folder(docs_path: str = "docs"):
    """
    Process all markdown files in the docs folder and create TF-IDF embeddings.
    """
    ...
    try:
        ...
        logger.info("TF-IDF embedding process completed successfully!")
        return True
...
if __name__ == "__main__":
    ...
    if not success:
        logger.error("Document TF-IDF embedding process failed.")
        sys.exit(1)
    else:
        logger.info("Document TF-IDF embedding process completed successfully!")